...
describe RssGenerator do
  ...
  describe ".add_show" do
    it "adds a show to the list of items" do
      show_time = Time.now
      subject.add_show(url: "https://example.com/show.mp3", title: "New Podcast Show", date: show_time, length_bytes: 1234)
      expect(subject.items.count).to eq 2
      last_item = subject.items.last
      expect(last_item.title).to eq "New Podcast Show"
      expect(last_item.enclosure.url).to eq "https://example.com/show.mp3"
  ...
  describe "save!" do
    it "saves the changes to the default file" do
      ...
      subject.add_show(url: "https://example.com/show.mp3", title: "New Podcast Show", date: Time.now, length_bytes: 1234)
